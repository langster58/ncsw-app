// Barrel export for the NCSW UI primitives.
// Section files should import from '@/ui'.

// Tokens
export * from './tokens'
export { FullWidthCopyContext } from './CopyContext'

// Atoms
export {
  IconArrow,
  IconCheck,
  IconChevron,
  IconChevronLeft,
  IconChevronRight,
  IconChevronUp,
  IconClose,
} from './Icon'
export { Image } from './Image'
export { Button } from './Button'
export { Chip } from './Chip'
export { Link } from './Link'
export { Eyebrow } from './Eyebrow'
export { Heading } from './Heading'
export { Lead } from './Lead'

// Molecules
export { Dropdown } from './Dropdown'
export { SectionIntro } from './SectionIntro'
export { Tag } from './Tag'
export { Metaline } from './Metaline'
export { DataList } from './DataList'
export type { DataRow } from './DataList'
export { SpecStrip } from './SpecStrip'
export type { SpecCell } from './SpecStrip'
export { PriceSummary } from './PriceSummary'
export { FilterChipGroup } from './FilterChipGroup'
export { NumberField } from './NumberField'
export { TextField } from './TextField'
export { ValueSlider } from './ValueSlider'
export { ControlColumn, CONTROL_BAND } from './ControlColumn'
export { PriceRangeSlider } from './PriceRangeSlider'
export { FilterTriggerButton } from './FilterTriggerButton'
export { ScoreMeter } from './ScoreMeter'

// Components
export { Container } from './Container'
export { Section } from './Section'
export { Card } from './Card'
export { Modal } from './Modal'
export { Accordion } from './Accordion'
export { NavBar } from './NavBar'
export { Shelf } from './Shelf'
export { Schematic } from './Schematic'
export type { SchematicNode } from './Schematic'
export { DataTable } from './DataTable'
export type { DataColumn } from './DataTable'

// Editorial / article
export { Byline } from './Byline'
export { Prose, proseSlug } from './Prose'
export type { ProseBlock } from './Prose'
export { CtaBar } from './CtaBar'

// Site mechanics
export { Reveal } from './Reveal'
export { Preloader } from './Preloader'
